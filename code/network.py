...
def handle_client_message(msg):
    ...
    try:
        ...
        if t == "shoot":
            ...
            v.players.append(
                Player(v.enemies_nicknames[-1], index=len(v.players), ip=data["ip"])
            )
            data = [(player.name, player.ip) for player in v.players]
...
def handle_server_message(msg):
    try:
        ...
        if data["type"] == "state":
            ...
            ips = [d[1] for d in data]
            v.players = []
            me_index = ips.index(list(v.client_socket.getsockname()))
            for idx in range(me_index, len(nicknames)):
                v.players.append(Player(nicknames[idx], index=idx, ip=ips[idx]))
...
def send_nickname():
    if v.client_socket and not v.client_socket._closed:
        payload = {
            ...
            "lobby": v.LOBBY,
            "nickname": v.nickname,
            "ip": v.client_socket.getsockname(),
        }
        v.client_socket.sendall(json.dumps(payload).encode("utf-8"))
