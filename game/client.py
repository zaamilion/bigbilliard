...
def setup_balls_16():
    ...
    v.LOSE = False
    v.players = []
    v.chat = [("Server", "Welcome to BigBilliard!")]
    v.current_player = 0
...
def game_loop():
    ...
    if (v.current_track != "game" or not pygame.mixer.get_busy()) and v.MUSIC_ENABLED:
        pygame.mixer.music.load(random.choice(v.battle_start_music))
        pygame.mixer.music.play()
        v.current_track = "game"
    while running:

        dt = v.CLOCK.tick(v.FPS)
        current_time = time.time()
        if (not pygame.mixer.music.get_busy()) and v.MUSIC_ENABLED:
            pygame.mixer.music.load(random.choice(v.battle_all_music))
            pygame.mixer.music.play()
        ...
        v.screen.blit(v.table_pattern_img, v.TABLE_RECT)
        ...
        if len(v.balls) <= 1:
            ...
            if v.ONLINE and v.AUTHED:
                ...
                db.db.update_balance(
                    v.username, db.db.get_balance(v.username) + v.players[0].score * 2
                )
            return "end"
